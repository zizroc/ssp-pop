...
p6 <- ggplot(data = sum_ssp_kcal_edu_macronutrients, 
       aes(x = year, 
           y = sum_mean_AMDR)) + 
...
```{r}
sum_ssp_kcal_edu_macronutrients %>% 
  dplyr::filter(SCENARIO == "SSP2_v9_130115") %>% 
  dplyr::mutate(low_AMDR = sum_mean_AMDR - sum_range_AMDR, 
                high_AMDR = sum_mean_AMDR + sum_range_AMDR)
```
...
diff_ssp_kcal_edu_macronutrients <- sum_ssp_kcal_edu_macronutrients %>% 
  dplyr::mutate(mean_AMDR_2020 = ifelse(year == 2020, 
                                        sum_mean_AMDR, 
                                        NA)) %>% 
  dplyr::group_by(SCENARIO, nutrient) %>% 
  tidyr::fill(mean_AMDR_2020) %>% 
  ungroup() %>% 
  dplyr::mutate(mean_AMDR_absdiff2020 = sum_mean_AMDR - mean_AMDR_2020) %>% 
  dplyr::filter(year %in% c(2050, 2075, 2100))
...
```{r}
p7 <- ggplot(data = diff_ssp_kcal_edu_macronutrients, 
       aes(x = as.factor(year), 
           y = mean_AMDR_absdiff2020, 
           fill = nutrient)) + 
  ...
  labs(
    title = "Difference in metabolic energy demand from 2020 by macronutrient", 
    x = "year", 
    y = "Mkcal per day"
  )
...
p8 <- ggplot(data = diff_ssp_kcal_edu_macronutrients %>% 
               dplyr::filter(SCENARIO == "SSP2_v9_130115"), 
       aes(x = as.factor(year), 
           y = mean_AMDR_absdiff2020*1e-6, 
           fill = nutrient)) + 
  geom_bar(stat="identity",  
           position = "dodge", 
           alpha = 0.9) + 
...
Planetary boundaries (Rochstrom et al.):
Earth system process: | Control variable: | Boundary (uncertainty range):
Climate change | GHG emissions | 5 Gt CO2e/y (4.7-5.4 Gt CO2e/y)
Land-system change | Cropland use | 13 M km^2 (11-15 M km^2)
Freshwater use | Water use | 2500 km^3/y (1000-4000 km^3/y)
Nitrogen cycling | N application | 90 Tg N/y (65-90 Tg N/y, or 90-130 Tg N/y)
Phosphorous cycling | P application | 8 Tg P/y (6-12 Tg P/y, or 8-16 Tg P/y)
