...
What is the maximum available supply of wind energy?
...
solar_potential <- read_csv(file = "~/Data/IEP_Model/data_iep_model/SolarGIS solar potential data - harmonized summary.csv", skip = 1)


```

The column labelled "Median" is the median theoretical potential (GHI) measured in kWh/m^2/day long-term. The column labelled "Median_1" is the median practical potential (PVOUT) for Level 1, measured in kWh/kWp/day long-term. Until I better understand how to use the PVOUT numbers, I will use the GHI numbers here.

```{r}
practical_solar_potential <- solar_potential %>%
  dplyr::rename(
    "iso3" = "ISO_A3",
    "country" = "Country or region",
    "evaluated_area_km2" = "Evaluated area",
    "level_1_area_ratio" = "Level 1 area \n(% of evaluated area)",
    "minimum_pvout" = "Minimum",
    "x10th_percentile_pvout" = "10th percentile",
    "x25th_percentile_pvout" = "25th Percentile",
    "mean_pvout" = "Average",
    "median_pvout" = "Median",
    "x75th_percentile_pvout" = "75th Percentile",
    "x90th_percentile_pvout" = "90th percentile",
    "maximum_pvout" = "Maximum"
  ) %>% 
  dplyr::select(iso3, country, evaluated_area_km2, level_1_area_ratio, minimum_pvout, x10th_percentile_pvout, x25th_percentile_pvout, minimum_pvout, median_pvout, x75th_percentile_pvout, x90th_percentile_pvout, maximum_pvout) %>% 
  dplyr::mutate(
    practical_pvout_MJ = evaluated_area_km2*level_1_area_ratio*median_pvout*(1e6)*365*3.6e6, 
    practical_pvout_EJ = practical_pvout_MJ*1e-12
  )

practical_solar_potential
