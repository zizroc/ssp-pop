...
```{r}
eatlancetdiet_target <- ssp2_kcal_eatlancetdiet %>% 
...
eatlancetdiet_animalfeeds <- eatlancetdiet_target_animalprod %>% 
  ...
  dplyr::select(EATlancet_group, year, subregionname, feed_tonnes)
...
eatlancetdiet_target_allprod <- eatlancetdiet_target_animalprod %>% 
  ...
  dplyr::select(-kcal_yr, -g_kcal), 
  by = c("year", "subregionname", "EATlancet_group", "tonnes"))
...
eatlancetdiet_summary <- eatlancetdiet_animalfeeds_1 %>% 
  dplyr::group_by(year, subregionname, feed_ingredient) %>% 
                            ...
                            "aves_meat", ifelse(
                              ...
                              "bovine_dairy", ifelse(
                                ...
                                "bovine_meat", ifelse(
                                  ...
                                  "fish_meat", 
                                  NA)))))))))))))), 
    by = c("year", "subregionname", "food_group")
  ) %>% 
  dplyr::mutate_each(funs(replace(., which(is.na(.)), 0))) %>%
...
A tropical livestock unit (TLU) is equivalent to 250 kg (liveweight). The following table is used for reference.
...
proc_per_animal
```
Original crop weight prior to food/feed processing. This uses an assumed value chain loss of 32%.
```{r}
...
eatlancetdiet_summary_2 <- eatlancetdiet_summary %>% 
  ...
  dplyr::mutate(crop_live_weight = ifelse(!is.na(scaling_factor), 
                                          ...
                                          NA)) %>% 
  dplyr::select(year, subregionname, food_group, crop_live_weight) %>% 
  tidyr::drop_na()
...
eatlancetdiet_summary_3 <- eatlancetdiet_summary_2 %>% 
  ...
  dplyr::select(year, subregionname, food_group, head_av)

head(eatlancetdiet_summary_3)
```

Assuming that one cow requires approximately 4 ha of pasture (including fallow land for animals with feed-subsidized diets), one sheep/goat requires 0.4 ha, one pig 0.2 ha, and one chicken 0.01 ha, I estimate the livestock pasture demand to be as follows:
...
pasture_demand_summary

#TODO figure out what the NA subregionname was that became a 0 somewhere above

```
#### Cropland demand

We need to consider how the yield gap will have closed by 2050. The yield in 2050 will determine how much cropland is required to produce the necessary food/feed crops.
...
Production = cropping_intensity.YA.cropland_area.(1-value_loss); cropland_area = production/(cropping_intensity.YA.(1-value_loss))
```{r}
...
cropland_area_demand <- eatlancetdiet_summary_2 %>% 
  ...
  tidyr::drop_na() %>% 
  dplyr::mutate(
    cropland_area = crop_live_weight/(CROPPING_INTENSITY*(1-value_chain_loss)*YA)
  ) %>% 
  dplyr::select(year, subregionname, food_group, management, cropland_area) %>% 
  tidyr::pivot_wider(., names_from = management, values_from = cropland_area) %>% 
  dplyr::rename(
...
cropland_pasture_area_summary <- cropland_area_demand %>% 
  dplyr::group_by(year, subregionname) %>% 
  dplyr::summarise(irrigated_area_ha = sum(irrigated_area_ha, na.rm = TRUE), 
                   rainfed_area_ha = sum(rainfed_area_ha, na.rm = TRUE), 
                   .groups = "drop") %>% 
  dplyr::full_join(pasture_demand_summary %>% 
                     dplyr::filter(subregionname != 0), 
                   by = c("year", "subregionname"))

head(cropland_pasture_area_summary)
...
These data represent FAOSTAT data for land-use from 2000 to 2018, so I label this explicitly.
```{r}
...
```
Now compare how cropland and pasture land-use in 2021 compares with that projected for 2050, based on the EAT-Lancet Planetary Health diet. Units are 1000 ha.
```{r}
...
irrigated_cropland_rel <- land_use_2000to2021 %>% 
  dplyr::filter(Year == 2021) %>% 
  dplyr::mutate(cropland_irr_rel = abs(cropland_irr)/cropland, 
                cropland_rfd_rel = abs(cropland_rfd)/cropland) %>% 
  dplyr::select(Year, subregionname, cropland_irr_rel, cropland_rfd_rel)

irrigated_cropland_rel
```


Cropland is defined in terms of 'cropland_min' (completely irrigated) and 'cropland_max' (completely rain-fed) demands; pasture is defined as above.
```{r}

landuse_2050 <- cropland_pasture_area_summary %>% 
  dplyr::mutate(
    cropland_min = irrigated_area_ha*1e-3, 
    cropland_max = rainfed_area_ha*1e-3, 
    pasture = pasture_ha*1e-3
  ) %>% 
  dplyr::full_join(landarea_regions, 
                   by = "subregionname")
...
Difference in pasture between 2021 and 2050.
```{r}

pasture_change <- landuse_2021_long %>% 
  dplyr::rename(year = Year) %>% 
  dplyr::filter(land_use == "pasture") %>% 
  dplyr::full_join(landuse_2050 %>% 
                     dplyr::select(year, subregionname, pasture) %>% 
                     tidyr::pivot_longer(., cols = pasture, names_to = "land_use", values_to = "kha"), 
                   by = c("year", "subregionname", "land_use", "kha")) 

head(pasture_change)

```
```{r}
ggplot(pasture_change, 
       aes(x = year, 
           y = kha)) + 
  geom_line() + 
...
Difference between cropland in 2021 and maximum cropland demand (100% rainfed) in 2050.
```{r}

cropland_change <- landuse_2021_long %>% 
  dplyr::rename(year = Year) %>% 
  dplyr::filter(land_use == "cropland") %>% 
  dplyr::full_join(landuse_2050 %>% 
                     dplyr::select(year, subregionname, cropland_max) %>% 
                     dplyr::rename(cropland = cropland_max) %>% 
                     tidyr::pivot_longer(., cols = cropland, names_to = "land_use", values_to = "kha"), 
                   by = c("year", "subregionname", "land_use", "kha")) 

head(cropland_change)

```

```{r}
ggplot(cropland_change, 
       aes(x = year, 
           y = kha)) + 
  geom_line() + 
  facet_wrap(~subregionname, 
