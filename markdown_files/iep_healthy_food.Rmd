...
p1a <- ggplot(interp_fit_data, aes(x = age, y = kcal_interp)) + 
  geom_line(aes(colour = activity_level, linetype = gender))
...
der_surpluses_regions$region_f <- factor(der_surpluses_regions$region_f, levels = c("South Asia", "Africa", "Southeast Asia", "Oceania", "South America", "Latin America and Caribbean", "MENA", "Central Asia and Russia", "East Asia", "Europe", "Australia and New Zealand", "North America"))
```
...
In order to make a one-to-one comparison across diet plans, I normalise daily kcal requirements to 2500 kcal/day/cap.
```{r}
...
all_plans_mapped <- all_plans_mapped %>%
  ...
  dplyr::mutate(normalisation = 2500 / kcal_day_total) %>%
...
Confirm that I'm using the vcl number correctly from Alexander.
...
global_crop_demand_food_feed <- global_crop_demand_food_feed %>% 
  dplyr::mutate(diet_plan_f = diet_plan)

global_crop_demand_food_feed$diet_plan_f <- factor(global_crop_demand_food_feed$diet_plan_f, levels = c("ELA-PHD", "CNS-CFP", "FDA-SDP"))
...
ymax <- (max(global_crop_demand_food_feed$tonnes_day) + 0.25 * max(global_crop_demand_food_feed$tonnes_day)) * 1e-6
ymin <- 0
...
pasture_demand <- livestock_quantity %>% 
  ...
  dplyr::mutate(forage_area_ha = head_animals*pasture_quality*forage_area_ha) %>% 
  dplyr::select(SCENARIO, iso_alpha3, subregionname, diet_plan, year, model_group, pasture_quality, forage_area_ha)
