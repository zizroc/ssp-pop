...
Daily energy requirements for humans from the FAO: [Joint, F.A.O. (2004). Human energy requirements. Report of a Joint FAO/WHO/UNU Expert Consultation, Rome, 17-24 October 2001.](http://www.fao.org/3/y5686e/y5686e00.htm).
...
These data contain daily energy requirements (MJ/day and kcal/day) for children (ages ) and adults (ages ) by age range, gender, body mass, and activity level. 
...
```{r}
# I use `drop_na` below because light and heavy kcal values are not given for children under 6 yrs old. In such a case, moderate values are passed.
...
TODO Check why ssp1... uses left_join and others use full_join here?
```{r}
ssp1_kcal_edu <- ssp_edu_long %>%
  dplyr::filter(SCENARIO == "SSP1_v9_130115") %>%
  dplyr::left_join(fit_data_final,
...
## Metabolic energy from FAO food security indicators

Suite of food security indicators from [FAOSTAT](http://fenixservices.fao.org/faostat/static/bulkdownloads/Food_Security_Data_E_All_Data_(Normalized).zip).

Trim the data to pull dietary energy supply (kcal/cap/day), average fat supply (g/cap/day), average protein supply (g/cap/day), and average supply of protein of animal origin (g/cap/day), average dietary energy requirement (kcal/cap/day), and minimum dietary energy requirement (kcal/cap/day).
...
Compare these food security indicators with those that I've calculated.
...
Compare for SSP2. Based on healthy diet considerations, for all age group and sex cohorts, the following surpluses (negative surpluses are deficits) of the daily energy requirements (kcal/cap/day) are given by country:
```{r}
...
save(paste0(data_path, "regions_map.csv"))
...
The national diet plans mapped to the standard food group typology follow.
```{r}
...
The USA-FDA plan advises a range of recommended daily quantities (here converted into kcal) for a range of recommended daily total kcal intakes. To compare the demands of all national plans, I will take the 2600 kcal per day numbers.
...
all_plans_mapped <- ela_phd_mapped %>%
  dplyr::full_join(
    fda_sdp_mapped %>%
      dplyr::filter(kcal_requirement == 2600) %>%
...
cns_cpf_wide %>% mutate(test = sum(across(.cols = cereals:vegetables))) %>% pull(test)
...
kcal_edu_cpf <- kcal_edu %>% 
  ...
  dplyr::summarise(across(cereals:vegetables, ~ sum(.x, na.rm = TRUE)), 
                   .groups = "drop") %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(animal_products = meat_poultry + eggs + dairy, 
                plant_products = cereals + fruits + legumes + oils + roots_tubers + vegetables) %>% 
  dplyr::select(SCENARIO, REGION, diet_plan, year, animal_products, fish_seafood, plant_products)
...
p4a <- ggplot(
  ...
  labs(title = "SSP1", y = "Relative to 2020 value") +
  facet_wrap( ~ year)
...
p4d <- ggplot(
  ...
  labs(title = "SSP4", y = "Relative to 2020 value") +
  facet_wrap( ~ year)
...
```{r}
land_use_regions %>% filter(year == 2018) 
```
...
<!--   dplyr::ungroup() %>%  -->
<!--   dplyr::select(iso_alpha3, subregionname, year, crop_lm_pred, crop_irr_lm_pred, crop_rfd_lm_pred, past_lm_pred, fore_lm_pred, othe_lm_pred) %>%  -->
<!--   dplyr::rename( -->
...
<!--   ) %>%  -->
<!--   tidyr::drop_na(iso_alpha3) # three countries/regions didn't linearly model -->
...
<!--                    by = c("iso_alpha3", "subregionname", "year", "cropland", "pasture", "forest", "otherland", "cropland_irr", "cropland_rfd")) %>%  -->
...
<!-- ```{r} -->
<!-- # for sure Andorra is wrong here. Instead of use these FAO numbers, I'm going to sum from the individual land use numbers. -->
<!-- # landareas <- land_use_2000to2021 %>% -->
<!-- #   dplyr::filter(year == 2021) %>% -->
<!-- #   dplyr::select(iso_alpha3, landarea) -->
...
All of the slick stuff above works for large regions but not for countries. I will just use land-use 2018 values.
```{r}
...
```{r}
# tmp <- cropland_food_1 %>%
#   dplyr::rename(tonnes_yr = tonnes_yr_food_crop) %>% 
#       dplyr::select(-tonnes_day_crop) %>%
#   dplyr::mutate(purpose = "food") %>%
#   dplyr::left_join(
#     cropland_feed_1 %>%
#       dplyr::rename(tonnes_yr = tonnes_yr_feed_crop) %>% 
#       dplyr::select(-tonnes_day) %>%
...
p6a <- ggplot(data = dat) + 
  ...
  geom_sf(data = dat %>% filter(diet_plan == "ELA-PHD" & kha_Qu4YG < 1 & SCENARIO == "SSP1_v9_130115"), aes(fill = kha_Qu4YG), lwd = 0.1) + 
  # coord_sf(crs = 54030) +
  cowplot::theme_minimal_grid() + 
  scale_fill_gradient(low = muted("blue"), high = muted("firebrick"), labels = scales::label_percent(accuracy = 1)) + 
  theme(
...
p6b <- ggplot(data = dat) + 
  ...
  # coord_sf(crs = 54030) +
  cowplot::theme_minimal_grid() + 
  scale_fill_gradient(low = muted("firebrick"), high = muted("violet"), labels = scales::label_percent(accuracy = 1)) + 
  theme(
    axis.text = element_blank(), 
    legend.title = element_blank(), 
    legend.text = element_text(size = 8), 
...
p6c <- ggplot(data = dat) + 
  geom_sf(color = "black", fill = NA, lwd = 0.1) + 
  geom_sf(data = dat %>% filter(diet_plan == "FDA-SDP" & kha_Qu1YG < 1 & SCENARIO == "SSP3_v9_130115"), aes(fill = kha_Qu1YG), lwd = 0.1) + 
  # coord_sf(crs = 54030) +
  cowplot::theme_minimal_grid() + 
  scale_fill_gradient(low = muted("firebrick"), high = muted("violet"), labels = scales::label_percent(accuracy = 1)) + 
  theme(
